...
def iterate(buff, depth, decrypt, check):

    if depth == 0:
        check(decrypt(buff))
        return
    for c in range(97, 123):
        print(type(buff), type(bytes(chr(c), 'utf-8')))
        iterate(
            buff.join(bytes(chr(c), 'utf-8')),
            depth - 1,
            decrypt,
            check
        )
...
def decrypt_password(supposed_key: bytes, message_to_decrypt=message):
    des = DES.new(supposed_key, DES.MODE_ECB)
    return des.decrypt(message_to_decrypt)


threads = []

for i in range(8, 9):
    thread = threading.Thread(target=iterate(key, i, decrypt_password, contains_a_swear_word))
